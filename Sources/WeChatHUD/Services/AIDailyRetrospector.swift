...
actor AIDailyRetrospector {
    ...
    private func call(_ userPrompt: String) async -> ModelResponse {
        ...
        do {
            let result = try await aiService.completeWithMetadata(
                ...
                user: userPrompt,
                options: CompleteOptions(timeout: 90, temperature: 0.2, maxTokens: 768, responseFormatJSON: true)
            )
    ...
    private func parse(_ raw: String) -> Retrospective? {
        AIJSONExtractor.decodeFirstObject(from: raw, as: Retrospective.self)
    }
