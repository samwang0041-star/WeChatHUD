...
actor VIPAggregator {
    ...
    func aggregate(
        ...
        let prompt = template
            ...
            .replacingOccurrences(of: "{mood_history}", with: recentMoodHistory.isEmpty ? "没有记录" : recentMoodHistory)
            .replacingOccurrences(of: "{last_interaction}", with: lastInteraction.isEmpty ? "没有记录" : lastInteraction)
