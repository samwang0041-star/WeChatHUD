...
struct ContactIdentityIndex {
    ...
    static let empty = ContactIdentityIndex(displayNameByUsername: [:], canonicalUsernameByAlias: [:])
    ...
    static func build(records: [Record]) -> ContactIdentityIndex {
        ...
        for record in records {
            ...
            let fallback = username.contains("@chatroom") ? "未命名群聊" : username
        ...
        return ContactIdentityIndex(displayNameByUsername: displayNames, canonicalUsernameByAlias: canonical)
    }
