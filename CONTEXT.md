...
**自动托管 (Autopilot)**:
AI 自动代管微信聊天回复的系统。核心目标：在用户不方便回复时（开会、睡觉、忙碌），让 AI 代为回复，且对方察觉不到是机器在代管。由 AutopilotService 编排，包含消息批处理、回复时机模拟（模拟真人打字间隔）、风格一致性评分、敏感词防护、会话账本（防止自我矛盾）。**目标模式：启动后全自动发送，无需人工确认**。置信度分三档：高（直接发）、中（AI 结合上下文生成缓兵之计）、低（read-no-reply），绝不中断等待用户。**回复质量要求**：缓兵之计由 AI 自由生成（非代码模板），结合具体对话上下文，避免同一联系人在短时间内收到重复话术。
