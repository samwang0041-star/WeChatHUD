...
一刀一节，新的写在最上面。
AI 服务总分 8.1、已到过 8.0，连续两刀 <0.3 后换到微信连接。微信连接总分 8.2、已到过 8.0，连续两刀 <0.3 后换到首次引导。首次引导总分 6.5、从未到过 8.0。待确认回复 7.8、从未到过 8.0，回扫时留下。自动回复设置 8.1、已到过 8.0 且连续两刀 <0.3。
...
| 9 | 首次引导 | in-progress | 6.5 | 页题已让给连接卡；底栏「连上微信后才能继续」仍和「下一步」抢话。从未到过 8.0 |
| 10 | 关注谁 | queued | — | |
