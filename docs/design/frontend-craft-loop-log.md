...
一刀一节，新的写在最上面。
AI 服务总分 8.1、已到过 8.0，连续两刀 <0.3 后换到微信连接。微信连接总分 6.6、从未到过 8.0，留下。待确认回复 7.8、从未到过 8.0，回扫时留下。自动回复设置 8.1、已到过 8.0 且连续两刀 <0.3。
...
| 8 | 微信连接 | in-progress | 6.6 | 卡上已收成只读取；更换账号仍是系统链接。从未到过 8.0 |
| 9 | 首次引导 | queued | — | |
| 10 | 关注谁 | queued | — | |
