...
一刀一节，新的写在最上面。
AI 服务总分 8.1、已到过 8.0，连续两刀 <0.3 后换到微信连接。微信连接总分 7.4、空气/主动词/短句都 ≥ 8，从未到过总分 8.0，留下。待确认回复 7.8、从未到过 8.0，回扫时留下。自动回复设置 8.1、已到过 8.0 且连续两刀 <0.3。
...
| 8 | 微信连接 | in-progress | 7.4 | 保存失败已是工作台回执；高级里「设置 AI」仍是系统链接。从未到过 8.0 |
| 9 | 首次引导 | queued | — | |
| 10 | 关注谁 | queued | — | |
