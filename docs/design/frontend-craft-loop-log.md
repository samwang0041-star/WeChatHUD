...
一刀一节，新的写在最上面。
AI 服务总分 8.1、已到过 8.0，连续两刀 <0.3 后换到微信连接。微信连接总分 8.1、已到过 8.0，空气/主动词/短句都 ≥ 8，总分未到 8.5，留下。待确认回复 7.8、从未到过 8.0，回扫时留下。自动回复设置 8.1、已到过 8.0 且连续两刀 <0.3。
...
| 8 | 微信连接 | in-progress | 8.1 | 轮询回执已在连接卡下；准备时「取消」仍是系统链接。已到 8.0，未到 8.5 |
| 9 | 首次引导 | queued | — | |
| 10 | 关注谁 | queued | — | |
