...
一刀一节，新的写在最上面。
AI 服务总分 8.1、已到过 8.0，连续两刀 <0.3 后换到微信连接。微信连接总分 8.2、已到过 8.0，连续两刀 <0.3 后换到首次引导。首次引导总分 8.0、已到过 8.0。待确认回复 7.8、从未到过 8.0，回扫时留下。自动回复设置 8.1、已到过 8.0 且连续两刀 <0.3。
...
| 9 | 首次引导 | in-progress | 8.0 | 名单空/败已能走下去；「已选 N 个对话」仍是系统字。已到 8.0，未到 8.5 |
| 10 | 关注谁 | queued | — | |
