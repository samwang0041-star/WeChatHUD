...
每次续跑只做 **一刀**：把一个真实页面再逼近一档工艺。不要问用户。做完写日志、独立 commit，停在可编译、可测试的树上。

远目标：队列 **1–13** 都 `done`（该页总分 ≥ 8.5，且主动词 / 空气 / 短句 ≥ 8）。14–16 不是完成条件。用户说停，整轮才停。
...
一轮对话 = 一刀。读选刀 → 打开该页 → 审计圈债 → 改 → 验绿 → 写日志 → commit → **停**。下一刀等 Goal 续跑，或用户说「下一刀」。

顺序续跑。不要 `sleep`、不要分钟级 `/loop`。用户插话时，把手头这刀收到可提交（或明确丢弃）再按新指示走。
...
完成：日志含 Before/After、改了什么、测了什么、下一刀是谁。这一轮到此。
...
不要因为过了一晚或已经砍了十几刀就把 Goal 标完成。整轮完成 = 状态板 1–13 都是 `done`。那时才结束 Goal。
