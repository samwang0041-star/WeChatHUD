...
## 2026-09-20 — 动效语言对齐 Emil Kowalski：撤销条离场不再 ease-in，紧急脉冲并进统一呼吸

### [Engineer] 用 emilkowalski/skills 的规则收口两条每天都会碰到的曲线

- **撤销条**：入场 `CompanionMotion.enter()`（250ms 强 ease-out），离场 `exit()`（160ms 强 ease-out）。原先离场走 `easeIn(0.2)`，把第一帧拖慢。
- **P0 脉冲**：改走 `CompanionMotion.pulse()`（easeInOut + autoreverses）。原先 `easeOut(1.4)` 且 `autoreverses: false`，每圈从 0.57 跳回。
- 门禁：`ChromeMotionHygieneTests` 禁止 Views 再写 ease-in；`CompanionMotionTests` 钉住 exit < enter 且不超过 300ms。
- `docs/design/ui-language.md` 不变量 5 补了这两条；AGENTS.md 加 UI craft 指针，后续改 Views 动效先加载 `emil-design-eng` / `apple-design`。

第二轮（同一天）：
- 岛下 toast 不再 `orderFront` 瞬切：`CompanionMotion.toastWindowAction` 区分首次入场 / 替换 / 离场 / 藏岛瞬切；入场从岛向下 scale 0.95。
- 今天、未回、待确认、洞察侧栏的卡片按压补上 `CompanionPressStyle`，与待办行对齐。

第三轮（同一天）：
- 草稿列表、承诺卡、雷达发现、注意力条、洞察总览折叠头/活跃聊天/焦点卡补上按压。
- 日报「标记完成」和忽略风险改按压；高亮/风险/已完成折叠用 `withMotion` 展开，不再只转动 chevron。

第四轮（同一天）：
- 工作台对话框入场/离场：`scale(0.95)+opacity`，160ms 强 ease-out；关闭钮补按压。
- 岛内「还有 N 条普通更新」、通知/群简报的稍后提醒菜单不再瞬切，走 rowExpand + islandDetailReveal。
- AI 设置模型列表展开走 drawer 曲线。

第五轮（同一天）：
- 对话详情「发送…」主按钮、返回/改名/生成建议/复制，以及关注列表行，补上按压。
- 今天页筛选切换与待办「很久没处理」展开不再瞬切。

第六轮（同一天）：
- 设置页切换从 0.22s 收到 0.16s，侧栏选中同步；一天几十次的导航不再比就地展开更慢。
- 详情窗关闭/查看/返回/开始整理补上按压。

第七轮（同一天）：
- 首次关注卡片、上手三步、准入模式行、关系雷达列表、状态栏刷新，补上按压。

第八轮（同一天）：
- 日报日期箭头/导出/刷新热区收到 22pt，补按压。
- 洞察页签、指南 FAQ 行、添加关注勾选、自动回复排除钮补按压。

第九轮（同一天）：
- 洞察总览刷新/复制、待办原文关闭钮热区收到 22pt，补按压。

第十轮（同一天）：
- 对话详情返回/改名、自动回复详情返回、待办回执关闭钮热区收到 22pt。

第十一轮（同一天）：
- 筛选胶囊统一走 pageChange（160ms），洞察页签和关注子页不再瞬切。
- 详情窗关闭钮热区收到 22pt。

第十二轮（同一天）：
- 岛内收件箱/待办切换和待办范围胶囊不再瞬切。
- 行内稍后提醒/忽略、对话框关闭热区收到 22pt。

第十三轮（同一天）：
- 引导页步骤切到 pageChange；详情提醒关闭和日报忽略风险热区从 22×18/20 收到 22×22。

第十四轮（同一天）：
- 调用点不再手写 0.15/0.18/0.20/0.25，一律走 CompanionMotion token；今天页筛选去掉和胶囊重复的一层动画。

第十五轮（同一天）：
- 日报空态「重新生成」和「复制小结」补上按压。

第十六轮（同一天）：
- 连接卡片下一步、关注子页返回、自动回复「查看待确认」补上按压。

第十七轮（同一天）：
- 今天页「全部 N 条」筛选和「查看我答应的事」行补上按压，和下面的快捷入口一致。

第十八轮（同一天）：
- 日报/洞察「查看待办」「还有 N 件」、回顾页「完成」补上按压。

第十九轮（同一天）：
- 待办回执「撤销」、今天页「撤销 / 知道了」补上按压，和岛上撤销同一套手感。

第二十轮（同一天）：
- 待办「看已处理的」和从岛跳进某个范围不再瞬切列表。

第二十一轮（同一天）：
- 连接页多账号列表用 drawer 展开/收起，账号行补按压。

第二十二轮（同一天）：
- 没回的「自定义日期」抽屉展开；今天页切「没回的 / 要回复 / 全部」整页跟 pageChange。

第二十三轮（同一天）：
- 上手 AI 内联展开、模型列表抽屉补过渡；刷新/展开钮补按压。Esc/回车收起模型列表不动画。

第二十四轮（同一天）：
- 连接/引导/选人/AI 测试/扫描状态行用 companionStatusReveal 淡入，不再啪一下报错。

第二十五轮（2026-09-21）：
- 工作台待办/承诺/待确认回执、日报与没回错误空态、洞察失败、准入/托管/同步/AI/提醒/macOS 体验/改名/回顾/群简报状态行，统一 companionStatusReveal + ease()，不再硬切。
- 键盘、搜索输入、系统 DisclosureGroup、句中链接仍不动画。

第二十六轮（同一天）：
- 承诺保存失败、日报/资料导出回执用 companionStatusReveal；今天页「查看消息原文」不再啪开。
- 岛 toast 关闭钮和今天页稍后提醒 ellipsis 热区收到 22pt；空态「查看已处理」和岛内待办展开走 islandDetailReveal。
- 待办「很久没处理，点开查看」补上按压。搜索清除、句中「查看原文」仍不改。

第二十七轮（同一天）：
- 岛内待办回执、今天页「已处理 / 稍后提醒」条、联系人读取/推断失败改走 companionStatusReveal。
- 洞察总览折叠内容不再硬切；待办「收起」与「点开查看」同一套按压。

第二十八轮（同一天）：
- 待办详情切「查看原文」走 pageChange，不再整页瞬切。
- 更正归属/查看原文、切到全记、待确认「查看聊天记录」、承诺「查看对话」补上按压。
- 日报工作台日期箭头热区收到 22pt。搜索清除和洞察句中链接仍不改。

第二十九轮（同一天）：
- 岛内回复建议的加载/结果/失败走 companionStatusReveal，不再啪换一行。
- 今天/没回卡片展开后的操作行跟 rowExpand；对话详情复制钮热区收到 22pt。
- 联系人「恢复」「取消静音」补上按压。

第三十轮（同一天）：
- 草稿保存/失败回执走 companionStatusReveal；岛内分析标题从加载切到结果不再硬切。
- 准入「恢复弹出 / 恢复」、同步「恢复默认」补上按压；复制成功勾走 ease。

第三十一轮（同一天）：
- 密钥路径错误行走 companionStatusReveal；岛上 PixelBuddy 悬停面板改 islandDetailReveal，不再向上位移。
- 改名「恢复微信原名」、导出「查看文件」、检查更新/打开发布页补上按压。

第三十二轮（同一天）：
- 权限复查回执、诊断复制反馈走 companionStatusReveal。
- macOS 权限/通知入口、草稿「复制 / 查看对话」、日报「打开结果」补上按压。

第三十三轮（同一天）：
- 对话详情发送回执走 companionStatusReveal；查看微信/核对/复制/查权限改按压，不再用句中 link。
- 今天页「关注谁 / 设置 AI / 重试整理」、上手卡片指南/重检/更多 AI、连接「更换微信账号」补上按压。

第三十四轮（同一天）：
- 草稿「修改已保存」戳走 companionStatusReveal；对话详情 AI 建议加载/空态不再硬切。

第三十五轮（同一天）：
- AI 设置测试结果卡走 companionStatusReveal，失败/成功不再硬切进供应商卡片。

第三十六轮（同一天）：
- 对话详情 AI 建议列表出现走 companionStatusReveal；「更换服务」改按压。

第三十七轮（同一天）：
- 分析页空态「去 AI 服务配置」、获取密钥入口改按压。

第三十八轮（同一天）：
- 今天页「检查连接 / 查看新消息 / 重试分析」补上按压。

第三十九轮（同一天）：
- 自动回复排除名单的删除钮热区收到 22pt。

第四十轮（同一天）：
- 日报高亮/风险/已完成折叠内容走 companionStatusReveal，不再只转 chevron。

第四十一轮（同一天）：
- 承诺卡展开内容、雷达发现的「意义/建议」、连接页多账号列表走 companionStatusReveal，不再只转 chevron 或靠父级 animation 默默淡入。
- 岛上头齿轮和自动驾驶图标热区收到 22pt。搜索清除、句中链接、全宽折叠头仍不改。

第四十二轮（同一天）：
- 待办空态在「级别收起了内容」时给出「切到全记」；没回失败给出「再试一次」，不再让人去点时间胶囊。
- 关系雷达区分「还没有数据」和「这个筛选是空的」：分别去聊天回顾 / 看全部。洞察总览没有关注时给出「关注谁」。

第四十三轮（同一天）：
- 岛上空态按卡点给出连接微信 / 关注谁 / 设置 AI / 查看待办，按钮单独可点，不再合成一条 VoiceOver。
- 待确认空态在未运行时给出「开始整理」（岛内详情原先把工具栏开关藏掉，却仍写「点开始整理」）。
- 待办空态补「检查连接 / 关注谁」；草稿空态补「打开今天」。

第四十四轮（同一天）：
- 日报忽略风险不再藏在悬停后面：22pt 关闭钮常驻，悬停只提高不透明度，VoiceOver 随时能点。
- 洞察没有时间线时，空态自己给出「分析 / 查看原文」；没有解读时给出「分析」。
- 承诺某个筛选为空、别处还有记录时，给出「看全部」。

第四十五轮（同一天）：
- 准入「群 @ 提醒」没有关注的群时，空态给出「关注谁」；灰掉的「添加」说明是还没关注还是都加过了。
- 关注列表筛选/搜索为空时给出「看全部」或「清除搜索」。向导第一步未连微信时，主按钮自己说出禁用原因。

第四十六轮（同一天）：
- 日报后一天/导出灰掉时说明是「已经是今天」还是「正在整理」；更新与 macOS 权限在演示界面灰掉时说出原因。
- 岛上空态「查看已处理」改成和连接/关注同一套行按钮，不再用白底 bordered。

第四十七轮（同一天）：
- 演示界面「选择 JSON」灰掉时说明不会选密钥文件。
- 自动回复排除名单没人可加时给出「关注谁」；向导选人搜索为空时给出「清除搜索」。

第四十八轮（同一天）：
- 日报来源未验证时，除了「重新生成」还有「检查连接」——没同步成功再生成不会变出消息。
- 周报完全空着时按卡点给出「检查连接 / 关注谁」；添加关注对话框搜空或筛空时给出「清除搜索 / 看全部」。

第四十九轮（同一天）：
- 待确认发送被静音拦住时，回执上直接「取消静音」，不再把人赶到另一页。
- 对话详情没有本地记录时给出「在微信中查看」。

第五十轮（同一天）：
- 「不看谁」空态给出「什么会提醒我」（全局忽略的添加入口）；「静音」空态给出「打开今天」去右键一条消息。
- 今天页「没回的」搜索为空时可以清除搜索。

第五十一轮（同一天）：
- 日报还没有小结且从未同步成功时，除了「重新生成」还有「检查连接」。
- 向导打不开设置或没保存进度时，错误旁给出「重试」，不再只让人去找菜单栏。

第五十二轮（同一天）：
- 连接主按钮在准备/同步中灰掉时说明正在等重新登录、读密钥还是读聊天，并补上转圈；更换账号同时带上原因。
- 洞察「开始分析」在进行中改成「正在分析…」，不再只是灰掉。

第五十三轮（同一天）：
- 待确认「确认发送 / 取消本条 / 立即发送」进行中改成「正在发送… / 正在取消…」，空回复会说明「先写回复」。
- 联系人「重新推断」进行中改成「推断中…」，和「开始推断」对齐。

第五十四轮（同一天）：
- 今天页「查看新消息 / 重试分析 / 重试整理」进行中改成「正在同步… / 分析中… / 整理中…」。
- 对话详情「发送… / 复制」在空回复时说明「先写回复」。

第五十五轮（同一天）：
- 确认发送对话框不再一点就关：发完才关，按钮改成「正在发送…」，发送中不能返回修改。

第五十六轮（2026-09-22 收尾）：
- 原文窗（待办/承诺的「查看原文」）读失败的空态补「再试一次」：任务键挂上重试计数，按下去真的回到「正在读取原文…」。
- 钉这条形状的门禁（`StaleViewShapeGatesTests`）改判语义：键必须含 `item.id`，且不许出现无键 `.task {`。

第五十七轮（2026-09-22）：
- 「读不到关注名单」不再变成屏幕上的事实：静音清单不再印出 wxid（改走 `whitelistAllRead` + `visibleName`，读不到就是「暂时读不到名字」）、诊断不再写「关注 0」、日报导出不再写「关注对象 (0)」、没回那一页不再打印「都回过了」。
- 扫描在白名单读不到时跳过本轮：空名单会让已关注的会话落进那条专门排除白名单的 autopilot 通路，把消息排进错误的管线并推水位。
- 判据都带正对照（读得到时必须是真名字/真数字），四处修复做了变异验证：退回旧写法全部转红。

第五十八轮（2026-09-22）：
- 回顾的范围补上第三种答案：关注名单读不到时不再以「0 个对话」收进历史（那行会声称用户谁也没关注），改成这次回顾不开始并说明原因。
- 修正 §249/§250 里「测试目标 0 警告」的错账：那两次是增量缓存没重编测试文件造成的假象；强制重编后暴露 14 条老警告，修掉 1 条，其余 13 条记进 §251。断言零警告必须先从干净状态重编。
