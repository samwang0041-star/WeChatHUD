...
- `/Users/lilei/wechatcli/repo/wechat_cli/keys/scanner_macos.py:119-153, 168-196`：首次材料来自外部 C 二进制扫描微信进程内存。遇到 `task_for_pid` 后调用 `_resign_wechat`；该函数在 `:70-116` 对 `/Applications/WeChat.app` 加入 `com.apple.security.get-task-allow` 后 ad-hoc 重签，并要求退出/重启微信后以 `sudo wechat-cli init` 重试。
