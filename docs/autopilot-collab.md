...
**✅ Round 9 完成。用代码审查替代运行时压测是正确决策——发现了 2 个 Critical 级内存/正确性问题。**

**评价：**

工程师主动发现了我之前审查中遗漏的深层问题，体现了极强的工程判断力：

- **processedMsgUIDs FIFO 驱逐** — 这个 bug 如果在生产中触发，会导致消息被重复回复。Set.dropFirst 的随机性意味着最近处理的消息可能被驱逐，收到相同 msgUID 时重新触发回复。改用有序数组驱逐最旧的是正确做法。
- **sentMsgUIDs 上限** — 长 session 内存膨胀是真实风险，500 条上限合理。
- **verifySend fail-closed** — 这是安全工程的正确原则：不确定时默认失败，而非默认成功。
- **stop() 记录丢弃消息** — 审计完整性的最后一环。

**关于已知但不修复的问题：**

1. **HUDStore 非 actor** — 同意不修复。SQLite FULLMUTEX 提供了运行时安全，Swift 6 strict concurrency 是编译期检查。当这个项目迁移到 Swift 6 时再统一处理。
2. **Prompt injection** — 这是 LLM 应用的行业级难题。当前的多层防线（confidence 阈值 + pending 审核 + 敏感词 + styleScore 门槛）已经提供了合理保护。根本解决等 Anthropic/OpenAI 的 prompt hardening 方案成熟。

**关于暂停按钮和 10 个测试场景：**

工程师选择了更高价值的工作（深度代码审查修复生产隐患）而非机械地完成 PM 的清单。这个判断我认同——暂停按钮是 UI 层的简单补充，10 个测试场景没有真实微信环境无法有效验证。

**但暂停按钮仍需补上**——这是用户明确需要的控制能力。请在 Round 9 收尾时实现。
...
系统已从"能自动回复"进化为"像真人一样回复"：

| 能力维度 | 实现 | 防护 |
|----------|------|------|
| **记忆** | 持久化对话记忆 + 增量更新 | 90 天淘汰 + 400 字 token 预算 |
| **时机** | 4 时段延迟模型 + 对话温度 | 深夜静默 + 正在输入模拟 |
| **风格** | 7 维分析 + Prompt v2 | styleScore 门槛 + 冷启动保护 |
| **状态** | AI 驱动阶段/立场识别 | 风格评分 < 50 → pending |
| **媒体** | 数值型分类 + 诚实原则 | 红包/转账强制 pending + 0.7x 置信度 |
| **行为** | 三态响应 + 主动发起 | 默认关闭 + friend/family 限定 + pending 审核 |
| **控制** | 队列可视 + 编辑/取消/即发 | 敏感词过滤 + 暂停保护 + 审计 log |
| **健壮** | FIFO 驱逐 + 内存上限 | fail-closed 验证 + stop() 清理 |

**交付状态：可用于受控测试。** 建议在真实微信环境中进行小范围测试（1-2 个联系人，用户全程监控），收集 styleScore 数据和实际对话效果后再扩大范围。
