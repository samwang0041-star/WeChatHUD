...
- 点击主体：显式主动模式，命中面积是整个 banner（文案、头像、空白都算），不是某个按钮。
  - `groupAt`：先展开就地 briefing 卡（「为什么 @ 你？」），卡内再进入 Detail。
  - 其他：直接进入 Detail。进入 Detail 后忽略 hover enter/exit，不能因 hover 自动 collapse。
- 点击关闭（✕）：只收起 banner，保留待办状态、不静音该对话；**不得**同时触发主体动作。
- 点击稍后（🕐）：展开稍后提醒时间菜单；**不得**同时触发主体动作。
- 图标按钮永远优先于主体动作：主体手势位于内容下层，卡片内所有非交互元素都不参与命中测试，因此一次点击只产生一个效果。按钮的**整个 24×22 图标框**（而不仅是字形墨迹）都必须可点击 —— `Image` 标签需要显式 `contentShape`，否则框内边角会穿透到主体动作，瞄准 ✕ 偏 2pt 反而会打开对话。
- 点击「去微信回复」（briefing 卡内，banner 本体没有该按钮）：继承全局打开微信行为，先 collapse HUD，再打开微信，并抑制 hover 反弹或直到鼠标离开 HUD 区域（当前 `collapseAndYield(duration: 4)`）。

banner 元素与文案（一个身份行 + 一条消息，没有第三行）：

- 身份行：`[头像] 发送者 @你 · 会话名 · 到达时间`。会话名只在**私聊**（会话名等于发送者）时省略 —— 群名恰好与某成员同名时仍须显示，否则用户无法判断是哪个群；提及胶囊是唯一强调色，只在该消息 @ 了你或所有人时出现，且必须区分二者（`@你` / `@全员`，见下）；整行不换行，空间不足时先截断群名，发送者与时间不截断。
- 提及胶囊不得把广播说成个人提及：`isAtMention` 对 `@所有人` / `@all` 同样为真（它回答的是「这条是否与我相关」，因此才允许打断），而正文里的 `@所有人` token 已被上游剥离，所以胶囊是唯一能纠正该读法的地方 —— 广播渲染 `@全员`，个人提及渲染 `@你`。
- 消息：主体，唯一使用 display 字号的元素，最多三行；换行符折叠为空格（不改写内容本身）。空消息不是媒体消息（媒体由解析器给出 `[图片]` 等占位符），而是内容读取失败，须按「内容无法显示」类文案呈现，不得报告为正常到达。
- 到达时间用相对表述（`刚刚` / `12 分钟前`），满一小时后退回 `clockLabel`；理由是可读性（「12 分钟前」回答等待时长，绝对时间让用户自己做减法），而非「只有它带日期」——同日的 `clockLabel` 输出 `今天 HH:mm`，并不含日期。
