...
- 点击关闭（✕）：只收起 banner，保留待办状态；**不得**同时触发主体动作。
- 点击稍后（🕐）：展开稍后提醒时间菜单；**不得**同时触发主体动作。
- 图标按钮永远优先于主体动作：主体手势位于内容下层，卡片内所有非交互元素都不参与命中测试，因此一次点击只产生一个效果。
- 点击「打开微信」（briefing 卡 / Detail 内，banner 本体没有该按钮）：继承全局打开微信行为，先 collapse HUD，再打开微信，并抑制 hover 反弹 1.5s 或直到鼠标离开 HUD 区域。

banner 元素与文案（一个身份行 + 一条消息，没有第三行）：

- 身份行：`[头像] 发送者 @你 · 会话名 · 到达时间`。私聊（会话名等于发送者）省略会话名；`@你` 是唯一强调色，只在该消息 @ 了你时出现；整行不换行，空间不足时先截断群名，发送者与时间不截断。
- 消息：主体，唯一使用 display 字号的元素，最多三行；换行符折叠为空格（不改写内容本身）。
- 到达时间用相对表述（`刚刚` / `12 分钟前`），超过一小时才退回 `clockLabel`。
