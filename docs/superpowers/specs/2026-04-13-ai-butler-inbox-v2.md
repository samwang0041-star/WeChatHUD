...
**列表刷新保护：**
- 有行展开时，后台 scan 完成后列表静默更新（新增/移除行正常处理）
- 当前展开的行保持展开状态不塌缩
- 展开行的数据变了（如 AI 摘要更新），只更新内容不改变展开/折叠状态
...
管家不只看文字，也看图片。当对方发来图片时，管家应该分析图片内容，结合上下文告诉用户"对方发了什么"。

**用户体验：**

对话上下文：你问"晚上吃了什么？"，对方发来一张图片。
...
**分析范围：仅新收到的图片。** 不回溯历史图片。每次 scan 检测到新的图片消息时触发分析。
...
算法：从微信存储定位图片文件
    ...
    │   Library/Application Support/com.tencent.xinWeChat/.../Message/MessageTemp/...
    │   图片文件名与 msgSvrId 或 local_id 关联
    │
    ├── 优先找缩略图（thumb），体积小，分析够用
    │   找不到缩略图则找原图
    │   找不到任何文件 → 降级为 "[图片]"
    │
    ▼
算法：构建上下文
    ├── 图片前的 3-5 条对话文字消息（理解上下文）
    ├── 发送人信息
    │
    ▼
AI（多模态）：分析图片 + 上下文
    ├── 输入：图片文件 + 上下文文字 + prompt
    ├── 输出：一句话描述（≤30字）
    │   例："回复你吃了什么：发了一张蛋炒饭的照片"
    │   例："发来一张合同截图，标注了修改条款"
    │   例："发了一张自拍，看起来心情不错"
    │
    ▼
结果写入 AI 摘要缓存 → InboxItem.aiSummary
```
...
struct InboxContext {
    ...
    // 8. 媒体内容
    let mediaType: DetectedMediaType?       // image / voice / video / file / link / nil
    let mediaFilePath: String?              // 图片/视频的本地文件路径（缩略图优先）
    let mediaContextMessages: [MessageInfo] // 媒体消息前的 3-5 条文字消息
    
    // 9. 链接内容（baseType=49 时）
    let linkTitle: String?                  // 链接标题
    let linkDescription: String?            // 链接描述
    let linkURL: String?                    // 链接地址
    let linkBodyText: String?              // 抓取的网页正文（前 2000 字）
}
```

**AI prompt 设计方向：**
```
你是用户的微信消息管家。对方发来一张图片。
结合对话上下文，用一句话（不超过30字）描述这张图片的内容和意图。
不要说"这是一张图片"，要说出图片里是什么、跟对话有什么关系。
如果图片内容和上下文无关，只描述图片内容即可。
```

**支持的媒体类型与处理方式：**

| 类型 | baseType | AI 分析 | 摘要示例 |
|------|---------|---------|---------|
| 图片 | 3 | 多模态视觉分析 | "发来一张蛋炒饭的照片" |
| 视频 | 43 | 取首帧 → 视觉分析 | "发了一段办公室的视频" |
| 语音 | 34 | 不分析（无转文字能力） | "[语音消息]"（保持原样） |
| 表情包 | 47 | 不分析 | "[表情]"（保持原样） |
| 文件 | 49+appType | 提取文件名 | "发来文件: Q3报告.xlsx" |
| 位置 | 48 | 提取地址文本 | "分享了位置: 北京市朝阳区..." |

### 4.8 链接/文章/公众号消息分析

对方发来一个公众号文章、网页链接、或小程序分享时，管家应该打开链接读取内容，告诉用户文章大意，而不是只显示标题。
...
```
🟡 李姐                                5分
   分享了一篇AI趋势文章：主要讲2025年企业AI落地的五个方向，跟你们团队在做的事相关
```

**数据管线：**

```
新链接消息到达（baseType=49, appType=5/链接 或 appType=33/小程序）
    │
    ▼
算法：从消息 XML 提取链接信息
    ├── title（标题）
    ...
    ▼
判断是否需要深度分析
    ├── 有完整 des（>50字）→ 直接用 des 生成摘要，不抓取
    ├── des 太短或没有 → 抓取网页内容
    │
    ▼
抓取网页内容（仅需要时）
    ├── HTTP GET url → 提取正文（去掉导航/广告/脚本）
    ├── 只取前 2000 字（够 AI 理解大意）
    ├── 超时 5s → 降级为只用 title+des
    │
    ▼
算法：构建上下文
    ├── 链接前的 2-3 条对话（理解为什么发这个）
    ├── 发送人角色
    │
    ▼
AI：生成摘要
    ├── 输入：文章正文(或des) + 标题 + 对话上下文
    ├── 输出：两部分
    │   ├── 摘要（≤50字）："主要讲企业AI落地的五个方向"
    │   └── 相关性判断："跟你们团队在做的事相关" / 无特别相关性则不加
    │
    ▼
组合为 AI 摘要
    "分享了一篇AI趋势文章：主要讲企业AI落地的五个方向，跟你们团队在做的事相关"
```
...
```
📋 情况
李姐分享了一篇《2025年AI趋势报告》。文章核心观点：
1. 企业 AI 从试点进入规模化阶段
2. Agent 和 RAG 是两大主流落地模式
3. 数据隐私成为最大挑战
这篇文章跟你们团队正在做的 AI 管家项目方向一致。

💡 建议
可以回复表示看过了，或标记稍后阅读。

回复
✅ "看了，第2点跟我们在做的很像，回头聊聊"      [选择]
   "谢谢分享，我抽空看看"                       [选择]
   "收到"                                      [选择]
...
**注意 `[打开原文]` 按钮**：点击在系统浏览器中打开链接 URL。这是链接消息特有的操作。

**支持的链接类型：**

| 类型 | appType | 处理方式 | 摘要示例 |
|------|---------|---------|---------|
| 公众号文章 | 5 | 抓取正文 → AI 摘要 | "分享了一篇关于...的文章，主要观点是..." |
| 网页链接 | 5 | 抓取正文 → AI 摘要 | "分享了一个网页：..." |
| 小程序 | 33/36 | 只用 title+des（无法抓取） | "分享了小程序「美团外卖」：..." |
| 文件 | 6 | 提取文件名+大小 | "发来文件: Q3报告.xlsx (2.3MB)" |
| 音乐 | 3 | 提取歌名+歌手 | "分享了一首歌：《XX》- 歌手名" |
| 视频号 | 51 | 只用 title | "分享了视频号内容：..." |

**降级策略：**
- 网页抓取失败 → 用 title + des 生成摘要
- title + des 都没有 → "[链接]"
- AI 分析失败 → 显示 title 原文
...
- 网页抓取最多 5s 超时
- 每次 scan 最多抓取 3 个新链接
- 抓取结果缓存（key = url hash），相同链接不重复抓取
- 与图片分析共享 AI 优先级队列

**降级策略：**
- AI 不支持视觉（模型无多模态能力）→ 降级为 "[图片]" + 上下文推断
  - 例：上文是"吃了什么"，图片无法分析 → "可能回复了你吃了什么（图片无法分析）"
- 图片文件找不到 → 降级为 "[图片]"
- 图片过大（>5MB）→ 压缩或降级

**性能考虑：**
- 图片分析比文字摘要慢（2-5s），使用与文字摘要相同的 AI 优先级队列
- 缓存 key 包含消息 ID，不重复分析
- 每次 scan 最多分析 3 张新图片，防止队列堆积

### 4.8 冷启动策略

新用户无历史数据时：
- AI 摘要使用通用 prompt，不依赖风格 profile 或互动历史
- 回复建议偏保守中性（不用特定语气）
- 算法层不受影响（优先级评分基于角色配置，不依赖历史）
- 每次用户选择/修改回复建议 → 风格 profile 快速收敛
- 预计一周后建议开始个性化
