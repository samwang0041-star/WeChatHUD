...
final class DailyReportWorkspaceContractTests: XCTestCase {
    ...
    func testSourceUnavailableRetryPressesQuietly() throws {
        ...
        XCTAssertTrue(unavailable.contains("今天还没读到聊天。"))
        XCTAssertTrue(unavailable.contains("这一天没有可写的小结。"))
        XCTAssertFalse(unavailable.contains("今日来源未验证"))
        XCTAssertFalse(unavailable.contains("exclamationmark.triangle"))
        XCTAssertFalse(unavailable.contains("foregroundColor(.orange)"))
