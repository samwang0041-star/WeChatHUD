...
final class ChatInsightEngineTests: XCTestCase {
    ...
    func testAIChatInsightNormalizesSelfAliasesInOutput() async throws {
        ...
        {
          "headline": "lilei 已配合推进排期",
          "topics": [{
            "name": "排期",
            "message_count": 2,
            "participant_count": 2,
            "summary": "张三问 lilei 排期",
            "status": "讨论中",
            "my_involvement": "lilei 承诺晚点给",
            "attitudes": {"lilei": "表面配合"},
            "cross_chats": []
          }],
          "decisions": [],
          "action_items": [{"what": "给排期", "who": "lilei", "deadline": null}],
          "mentions_me": 1,
          "waiting_for_me": [{"source": "张三", "what": "等 lilei 给排期", "waiting_hours": 0}],
          "my_commitments": ["lilei 晚点给"],
          "needs_my_attention": true,
          "overall_mood": "正式",
          "mood_shift": null,
          "attitudes": [{"person": "lilei", "topic": "排期", "attitude": "表面配合", "evidence": "lilei: 我晚点给"}],
          "tone_changes": [],
          "signal_noise_ratio": 0.8,
          "decision_efficiency": "正常",
          "importance_to_me": {"level": "中", "reason": "张三在等 lilei"},
          "participants": [{"name": "lilei", "message_count": 1, "role": "执行者", "doing": "给排期", "attitude_toward": {"排期": "表面配合"}}],
          "relationship_signal": "平稳",
          "symmetry": 0.5,
          "cross_chat_topics": [],
          "insight": "lilei 被点名后没有立即给排期",
          "suggestion": "lilei 应回复张三"
        }
        """)
        ...
        let result = await insight.analyzeChat(
            ...
            timeRange: "今天",
            messages: [
                (sender: "张三", body: "@lilei 排期定了吗", time: 1000),
                (sender: "我（李雷）", body: "我晚点给", time: 1001),
            ],
            recalledMessages: [],
            memory: ""
        )
