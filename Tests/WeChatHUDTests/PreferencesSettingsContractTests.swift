...
final class PreferencesSettingsContractTests: XCTestCase {
    ...
    func testMotionRowWaitsBehindADisclosure() throws {
        let source = try MacExperienceSettingsSource.load()
        let sectionStart = try XCTUnwrap(source.text.range(of: "SettingsSection(\"macOS 体验与权限\")"))
        let disclosureStart = try XCTUnwrap(source.text.range(of: "DisclosureGroup(\"还要管通知\")"))
        ...
        XCTAssertTrue(firstScreen.contains("微信操作权限"))
        XCTAssertFalse(firstScreen.contains("系统通知"))
        XCTAssertFalse(firstScreen.contains("动画与透明度"))
        XCTAssertTrue(source.text.contains("还要管通知"))
